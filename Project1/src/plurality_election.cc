...
void PluralityElection::RunElection(VotingInfo* votingInfo) {
    ...
    election_record = new PluralityElectionRecord(candidates_list,
                                                  ballots_list);
...
void PluralityElection::DisplayResults(PluralityElectionRecord* election_record,
    ...
    while (!winners_list.empty()) {
        ...
        percent = percent*100;
        std::cout << i << ". " << current_candidate->GetName()
                  << "    " << candidate_num_ballots << "    "
                  << percent << "\n" << std::flush;
