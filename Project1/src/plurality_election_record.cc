...
PluralityElectionRecord(std::list<Candidate*> candidates,
...
void PluralityElectionRecord::DistributeBallots() {
    ...
    while (!nonDistributedBallotList_.empty()) {
        ...
        for (it = this->nonElectedCandidateList_.begin();
             it != this->nonElectedCandidateList_.end(); it++) {
...
void PluralityElectionRecord::
MoveFirstNCandidatesFromNonELectedListToWinnersList(int N) {
...
bool PluralityElectionRecord::CandidateNumBallotsComp(Candidate* candidate1,
