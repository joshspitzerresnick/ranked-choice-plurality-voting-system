...
STVElection::STVElection(VotingInfo* votingInfo) {
  ...
  stvElectionRecord_ = new STVElectionRecord(votingInfo->GetSTVCandidateList(),
                                             votingInfo->GetBallotList(),
                                            droop);
  // stvResultDisplay_= new ResultDisplay(); // NO LONGER USING - Josh
}

void STVElection::RunElection() {
  STVCandidate* candidate;  // stv candidate object pointer to hold candidate \
                               object to pass between member functions
  std::list<Ballot*> ballotList;  // ballot pointer list to hold ballots for \
                                     passing between stvelectionrecord functions
  ...
  while (true) {
    ...
    // Sort non-elected candidate list by number of votes, break tie \
       (embeded) if number of votes are equal
    stvElectionRecord_->SortNonElectedCandidateList();
    // Put the candidate with the least votes onto losers list and put \
       his/her ballots into non-distributed balots list
    candidate = stvElectionRecord_
  ...
  // if need more candidates to fill seats, move the candidates being put \
     on losers list last to winners list
