...
TEST_F(PluralityElectionRecordTests, SortNonElectedCandidateList) {
	...
	std::list<Candidate*> temp_candidate_list = election_record->GetNonElectedCandidateList();
...
TEST_F(PluralityElectionRecordTests, SortNonElectedCandidateList_reorder) {
    ...
    election_record_reorder = new PluralityElectionRecord(candidates_reorder, ballots_reorder);



    election_record_reorder->DistributeBallots();
	election_record_reorder->SortNonElectedCandidateList();
	std::list<Candidate*> temp_candidate_list = election_record_reorder->GetNonElectedCandidateList();
	Candidate* most_ballots = temp_candidate_list.front();
...
TEST_F(PluralityElectionRecordTests, MoveFirstNCandidatesFromNonElectedListToWinnersList) {
    ASSERT_ANY_THROW(election_record->MoveFirstNCandidatesFromNonELectedListToWinnersList(20));
